...
config.hosts.anonymousdomain = 'guest.xmpp.test';
...
config.bosh = 'https://bosh.test/http-bind';`)),
	...
	if cfg.xmppDomain != "xmpp.test" || cfg.streamDomain != "guest.xmpp.test" ||
		cfg.mucDomain != "conference.test" || cfg.websocket != "wss://ws.test/xmpp-websocket" ||
		cfg.bosh != "https://bosh.test/http-bind" {
